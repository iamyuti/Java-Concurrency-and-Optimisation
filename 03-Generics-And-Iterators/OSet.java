 ...
 */
public class OSet<E> implements OrdSet<E, OMSet<E>>  {
    ...
    /**
     * Konstruktor: Erstellt ein neues OSet.<br>
    ...
    /**
     * Die Methode gibt null oder ein Objekt zurück,
     * das sowohl den Typ Ordered als auch Modifiable hat, jeweils mit
     * passenden Ersetzungen der Typparameter, wenn x in der durch this bestimmten Ordnung
     * vor y kommt, andernfalls null. <br>
     ...
     * @return Objekt, das sowohl den Typ Ordered als auch Modifiable hat, andernfalls null.
     * Das Ergebnis hängt nur von Werten in this, x und y ab.
     */
    @Override
    public OMSet<E> before(E x, E y) {
        if (x == null || y == null) {
        ...
        // Wenn x vor y kommt: Rückgabeobjekt erzeugen:
        // Neues leeres OMSet erzeugen
        OMSet<E> result = new OMSet<>();

        // 1. Alle Knoten zwischen x und y (exklusiv) finden und ins OMSet einfügen
        Node<E> cur = first;
        while (cur != null) {
            if (cur != nx && cur != ny
                    && reachable(nx, cur)
                    && reachable(cur, ny)) {
                // add gibt ein neues OMSet zurück
                result = result.add(cur.element);
            }
            cur = cur.nextElement;
        }

        // 2. Alle Kanten zwischen diesen Knoten im OMSet nachbauen
        cur = first;
        while (cur != null) {
            if (cur != nx && cur != ny
                    && reachable(nx, cur)
                ...
                while (edge != null) {
                    Node<E> succNode = edge.node;
                    if (succNode != nx && succNode != ny
                            && reachable(nx, succNode)
                            && reachable(succNode, ny)) {
                        // Beide Knoten liegen im Bereich -> Ordnungsbeziehung in OMSet eintragen
                        result.setBefore(cur.element, succNode.element);
                    }
                    edge = edge.nextEdge;
                }
            }
     ...
     * erlaubter Ordnungsbeziehungen verwendet wird, welches ein Untertyp von Ordered<E, ? super E, ?> ist.
    ...
    public void check(Ordered<? super E, ?> newC) {
        ...
        // Wenn wir hier sind, ist alles ok -> c aktualisieren
        this.c = newC;
    }
